...
(function (sap) {
    var fioriToolsGetManifestLibs = function (manifestPath) {
        ...
        function getKeys(libOrComp, libOrCompKeysString) {
            Object.keys(libOrComp).forEach(function (libOrCompKey) { // ignore libs or Components that start with SAPUI5 delivered namespaces
   ...
   */
    sap.registerComponentDependencyPaths = function (manifestPath) { /*eslint-disable semi, consistent-return*/
...
sap.registerComponentDependencyPaths(manifestUri).catch(function (error) {
...
}). finally(function () { // setting the app title with internationalization
    ...
    if (componentName && componentName.length > 0) {
        ...
        } else { // Requiring the ComponentSupport module automatically executes the component initialisation for all declaratively defined components
        ...
        sap.ui.getCore().attachInit(function () { // initialize the ushell sandbox component
            sap.ushell.Container.createRenderer().placeAt("content");
        });
    }
});
